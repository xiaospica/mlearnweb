...
- 将历史持仓浏览从 `MlMonitorPanel` 解耦到策略详情主视图，避免用户只能在机器学习策略监控页签中查看历史持仓。
...
- 前端保持实盘工作台风格：`StrategyPerformanceSummaryCard` 使用紧凑 KPI 栅格，不做大面积装饰；`HistoricalPositionsCard` 继续兼容 `MlMonitorPanel` 传入 `historyDates` 的老用法。
...
- 前端：
  ...
  - 改造 `HistoricalPositionsCard`：未传 `historyDates` 时自行查询日期；详情页主 tab 直接渲染历史持仓卡片。

## 验收标准

- 策略详情主 tab 的顺序为：指标总览 → 收益曲线 → 当前持仓 → 历史持仓 → TopK/成交/除权/参数等现有卡片。
