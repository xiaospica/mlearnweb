...
## 当前进度（2026-05-10 整改实现后状态）— P0/P1 本机验收通过，待干净服务器 smoke

**结论**：mlearnweb 的独立部署骨架已完成生产化补强：依赖补齐、DataRoot 配置落点、双服务监听策略、Workbench/训练侧配置化、MLRUNS 降级、部署 health/smoke 入口均已落地；本机验证已通过 backend tests 与 frontend build。正式交付前仍需在一台干净 Windows Server 上执行 `install_all.ps1 -DataRoot D:\mlearnweb_data` 做最终 smoke 验收。

**状态口径**：
- Phase 1-3 的核心解耦方向基本成立：vnpy 实盘侧主链路已转为 HTTP fanout，不再 import vnpy 模块。
- Phase 4 部署脚手架已补齐生产化关键项：`install_all.ps1` 生成 `DataRoot\config\.env`，NSSM 注入 `MLEARNWEB_ENV_FILE`，SQLite/uploads/logs/nodes/dist 收敛到 DataRoot 或明确绝对路径。
- P0/P1 代码整改已完成；P2 已完成 `/health` 部署自检和 `deploy/smoke_test.ps1`。P3 仍按后续优化排期。
- 发布闸门：本地 `backend tests` 与 `frontend build` 已通过；“可正式部署/一键快速部署能力”的最终标记仍以干净 Windows Server smoke 通过为准。
...
flowchart LR
    ...
    P4[Phase 4<br/>Windows 快速部署骨架<br/>✓ mlearnweb 70eb595<br/>已生产化补强]:::done
    P0[P0 发布阻断<br/>依赖/配置/监听/测试<br/>✓ 本机验收]:::done
    P1B[P1 正式部署必需<br/>配置收敛/训练侧解耦<br/>✓ 本机验收]:::done
...
mlearnweb backend 测试套历史记录：**160 全过**（128 原有 + 13 新增 historical_metrics_sync + 6 stock_name_cache + 13 其他）。2026-05-10 复核时重新执行 `tests/test_backend`，剥离本机临时目录权限噪音后仍有 **8 个真实失败**，已在本次 P0-4 中修复；当前验证结果为 `151 passed, 9 skipped`。
...
| 优先级 | 状态 | 本次落地 |
|---|---|---|
| P0 | ✅ 已完成本机验收 | requirements 补齐；`install_all.ps1` 自动写 DataRoot `.env` 并安装 `vendor/qlib_strategy_core`；research `0.0.0.0:8000`、live `127.0.0.1:8100`；`corp_actions` 与 `replay_equity_sync` 测试恢复全绿 |
| P1 | ✅ 已完成本机验收 | `STRATEGY_DEV_ROOT` 未配置时禁用 Workbench 创建/入队/启动；前端训练模板去除开发机绝对路径，改用 `<STRATEGY_DEV_ROOT>` 占位并由后端替换；`MLRUNS_DIR` 未配置时研究页降级，不阻断 live-trading |
| P2 | ✅ 已完成关键入口 | `/health` 返回 frontend、DB、uploads、vnpy nodes、mlruns、live proxy 自检；新增 `deploy/smoke_test.ps1` 覆盖 backend import、frontend build、health |
| P3 | ⏳ 未开始 | 大 chunk 拆分、HTTPS/Caddy/autossh、运维 playbook 继续后排 |

本机验证：
- `python -m pytest tests/test_backend -q --basetemp .tmp_pytest` → `151 passed, 9 skipped`
- `cmd /c npm run build` → 通过；仍有 Vite 大 chunk warning，归入 P3

剩余发布闸门：干净 Windows Server 上运行 `install_all.ps1 -DataRoot D:\mlearnweb_data` 并访问 `http://<server-ip>:8000/`。
